...
export function AlterarStatus({
  ...
  async function alterarStatus(novoStatus: string) {
    ...
    setShowMenu(false)
    try {
      await fetch(`/api/usicrom/${orcId}/status`, {
        method: 'POST',
    ...
    <div className="relative">
      ...
      {showMenu && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-20 bg-bg-card border border-bg-border rounded-lg shadow-xl min-w-[160px] overflow-hidden">
            {proximos.map(status => {
